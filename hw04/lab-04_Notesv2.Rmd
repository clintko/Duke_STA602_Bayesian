...
Teacher gives two tests to all the students
data = difference in teh test scores
objective: is there a difference in teh performance between ad S and C

`#S` = 20% of the class (random)
`#C` = 80% of the class (random)
...
an easy way is to use R base plot
```
1. hist(S)
2. hold on
3. hist(C)
...
xLimits = seq(min(iqData$Gain) - (min(iqData$Gain) %% 5),
              ...
              by = 5)
print(min(iqData$Gain))
print(min(iqData$Gain) %% 5) # ((-33) - r) %% 5 == 0 => r = 2
print(xLimits)
```

Note:
notice that different treatment is plotted side by side instead of laying on top of each other.

here for each bin, it is plotting controls and spurters separately.
